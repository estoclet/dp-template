 ...
 */
class MiniSitePageController extends EntityController {
   ...
   *   The request stack.
   */
  public function __construct(EntityTypeManagerInterface $entity_type_manager, EntityTypeBundleInfoInterface $entity_type_bundle_info, EntityRepositoryInterface $entity_repository, RendererInterface $renderer, TranslationInterface $string_translation, UrlGeneratorInterface $url_generator, RequestStack $request_stack) {
  ...
  public function addPage($entity_type_id) {
    ...
    $miniSiteId = $this->requestStack->getCurrentRequest()->get('mini_site');
    $miniSiteEntity = MiniSite::load($miniSiteId);
    if(!$miniSiteEntity) {
      throw new NotFoundHttpException();
    }
    $miniSiteTypeEntity = MiniSiteType::load($miniSiteEntity->bundle());
    $allowedBundles = array_flip($miniSiteTypeEntity->get('allow_page_type'));
    if(isset($allowedBundles[0])) {
      unset($allowedBundles[0]);
    }
    if(!empty($allowedBundles)) {
      $bundles = array_intersect_key($bundles, $allowedBundles);
    }
    if($miniSiteEntity->menu_type->value == 'anchor') {
      $bundles = ['link' => $bundles['link']];
    }

    \Drupal::moduleHandler()->alter('mini_site_page_allowed_type', $bundles, $miniSiteEntity);
    ...
    if (count($bundles) == 1) {
      ...
      return $this->redirect($form_route_name, [$bundle_argument => $bundle_name, 'mini_site' => $this->requestStack->getCurrentRequest()->get('mini_site')]);
    }
    ...
    foreach ($bundles as $bundle_name => $bundle_info) {
      $build['#bundles'][$bundle_name] = [
        ...
        'add_link' => Link::createFromRoute($bundle_info['label'], $form_route_name, [$bundle_argument => $bundle_name, 'mini_site' => $this->requestStack->getCurrentRequest()->get('mini_site')]),
      ];
    }
