...
var AccessConfig = (function($) {
	...
	var config = {
      ...
      subTitle: {
			  lang: {
          en: 'Configure accessibility settings',
	...
	function settingForm(){
		...
		if( global.userParams.Modal == true || global.userParams.Modal === undefined) {
			...
			//subTitle
      var subTitle = $create({tagName:'div', id:userPrefix+'-'+config.ModalContainer.subTitle.id, class:config.ModalContainer.subTitle.class});
      subTitle.textContent = config.ModalContainer.subTitle.lang[ langRef ];
      titleWindow.appendChild( subTitle );
    ...
    if(global.userParams.Reinit != false){
      var fieldset = $create({tagName:'fieldset', id:userPrefix+'-'+config.ReinitFieldset.id});
      var reInitBtn = $create({tagName: 'button', id: config.ReinitButton.id, class: config.ReinitButton.class});

      reInitBtn.textContent = config.ReinitButton.lang[ langRef ];

      reInitBtn.addEventListener('click', function(event) {
        var $fieldContentWrapper = $('.a42-ac-content');
        var $fieldSet = $fieldContentWrapper.find('.a42-ac-fieldset:not(:last-child)');

        $fieldSet.each(function(idx, element) {
      ...
      fieldset.appendChild( reInitBtn );
      fieldsetContent.appendChild( fieldset );
    }
