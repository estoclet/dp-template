...
namespace Drupal\entity_dynamic_rendering\Plugin\Field\FieldFormatter;
 ...
 *
 * @FieldFormatter(
 *   id = "entity_reference_revision_dynamic_rendering",
 *   label = @Translation("Dynamic rendered entity"),
 ...
 * )
 */
class EntityReferenceRevisionDynamicRenderingFormatter extends EntityReferenceRevisionsFormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($this->getEntitiesToView($items, $langcode) as $delta => $entity) {
      $view_mode = $this->getSetting('view_mode');
      foreach($entity->getFieldDefinitions() as $field_name => $field_object) {
        if($field_object->getType() == 'view_mode_item') {
          $field_value = $entity->{$field_name}->getValue();
          $view_mode = $field_value[0]['value'];
          break;
        }
      }
