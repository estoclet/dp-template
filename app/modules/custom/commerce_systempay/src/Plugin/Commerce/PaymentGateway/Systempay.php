...
use Drupal\commerce_systempay\Tools;
use Drupal\commerce_price\Price;

abstract class Systempay extends OffsitePaymentGatewayBase
{
    /**
     * {@inheritdoc}
     */
    ...
    public function buildConfigurationForm(array $form, FormStateInterface $form_state)
    {
        ...
        require_once(drupal_get_path('module', 'commerce_systempay') . '/includes/SystempayApi.php');

        $form['#attached']['library'] = 'commerce_systempay/admin';
        ...
        if (function_exists('system_get_info')) {
            $info = system_get_info('module', 'commerce_systempay');
        } else {
            $info = \Drupal::service('extension.list.module')->getExtensionInfo('commerce_systempay');
        }
        ...
        $filenames = glob(drupal_get_path('module', 'commerce_systempay') . '/installation_doc/' . Tools::DOC_PATTERN);
        ...
        if (! empty($doc_files)) {
            $doc = '<span class="systempay-doc-links">' . $this->t('Click to view the module configuration documentation :');
            foreach ($doc_files as $file => $lang) {
                $doc .= '<a href="' . base_path() . drupal_get_path('module', 'commerce_systempay') . '/installation_doc/' . $file . '" target="_blank">' . $lang . '</a>';
            }
        ...
        $form['gateway_access']['site_id'] = [
            '#type' => 'textfield',
            '#systempay_field' => true,
            '#title' => $this->t('Shop ID'),
            '#description' => $this->t('The identifier provided by Systempay.'),
        ...
        if (! Tools::$pluginFeatures['qualif']) {
            $form['gateway_access']['key_test'] = [
                ...
                '#description' => $this->t('Key provided by Systempay for test mode (available in Systempay Back Office).'),
        ...
        $form['gateway_access']['key_prod'] = [
            ...
            '#description' => $this->t('Key provided by Systempay (available in Systempay Back Office after enabling production mode).'),
        ...
        $form['gateway_access']['sign_algo'] = [
            ...
            '#description' => $this->t('Algorithm used to compute the payment form signature. Selected algorithm must be the same as one configured in the Systempay Back Office.<br /><b>The HMAC-SHA-256 algorithm should not be activated if it is not yet available in the Systempay Back Office, the feature will be available soon.</b>'),
            '#options' => [
                'SHA-1' => 'SHA-1',
        ...
        $form['gateway_access']['ipn_url'] = [
            ...
            '#description' => '<span class="systempay-ipn-desc">
                    <img src="' . base_path() . drupal_get_path('module', 'commerce_systempay') . '/images/warn.png">' .
                    $this->t('URL to copy into your Systempay Back Office > Settings > Notification rules.') .
                '</span>',
        ...
        $languages = array_map([$this, 't'], \SystempayApi::getSupportedLanguages()); // translate language labels
        ...
        $form['payment_page']['capture_delay'] = [
            ...
            '#description' => $this->t('The number of days before the bank capture (adjustable in your Systempay Back Office).'),
        ...
        $form['payment_page']['validation_mode'] = [
            ...
            '#description' => $this->t('If manual is selected, you will have to confirm payments manually in your Systempay Back Office.'),
            '#options' => [
                '' => $this->t('Systempay Back Office configuration'),
                '0' => $this->t('Automatic'),
                '1' => $this->t('Manual')
            ],
    ...
    public function validateConfigurationForm(array &$form, FormStateInterface $form_state)
    {
        ...
        require_once(drupal_get_path('module', 'commerce_systempay') . '/includes/SystempayRequest.php');

        $grouped_values = $form_state->getValue($form['#parents']);
        $request = new \SystempayRequest(); // new instance for parameters validation
        ...
        foreach ($grouped_values as $key1 => $group) {
            ...
            foreach ($group as $key2 => $value) {
                ...
                if (! $field || ! isset($field['#systempay_field']) || ! $field['#systempay_field']) {
                    continue;
                }
    ...
    public function onReturn(OrderInterface $order, Request $request)
    {
        ...
        require_once(drupal_get_path('module', 'commerce_systempay') . '/includes/SystempayResponse.php');

        // get logger instance
        $logger = \Drupal::logger('commerce_systempay');
        ...
        if (! $response->isAuthentified()) {
            ...
            $logger->error('Signature algorithm selected in module settings must be the same as one selected in Systempay Back Office.');
            ...
            if ($result) {
                ...
                if ($this->configuration['gateway_access']['ctx_mode'] === 'TEST') {
                    ...
                    if (\Drupal::state()->get('system.maintenance_mode')) {
                        ...
                        $message = $this->t('The automatic validation has not worked. Have you correctly set up the notification URL in your Systempay Back Office ?');
                        $message .= '<br />';
    ...
    private function savePayment($order, $response)
    {
        ...
        if ($this->pluginId === 'systempay_multi') {
        ...
        $currency = \SystempayApi::findCurrencyByNumCode($currency_code);
