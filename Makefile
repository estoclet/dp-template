...
# Variables with spaces in .env are ok for Make but not for vlucas/phpdotenv
# and with quotes it does not work anymore in Make.
...
docker-shell-web: ## Open a command line in the web container.
	@$(DOCKER_COMPOSE_COMMAND) exec -e LINES=$(tput lines) -e COLUMNS=$(tput cols) web bash

.PHONY: docker-shell-web-cmd
docker-shell-web-cmd: ## Execute the entered command. Example: make docker-shell-web-cmd "ls -l"
	@$(DOCKER_COMPOSE_COMMAND) exec -T web bash -c "$(filter-out $@,$(MAKECMDGOALS))"

.PHONY: docker-drush
docker-drush: ## Execute the entered Drush command. Example: make docker-drush cr
	@$(DOCKER_COMPOSE_COMMAND) exec web bash -c "$(DRUSH) $(filter-out $@,$(MAKECMDGOALS))"
...
docker-site-install: ## Install Drupal website. Example: make docker-site-install SELECTED_SITE=all
	...
	@$(DOCKER_COMPOSE_COMMAND) exec web bash -c "cd $(PROJECT_PATH) && ./scripts/install-dev.sh ${SELECTED_SITE}"
...
docker-site-update: ## Update Drupal website. Example: make docker-site-update SELECTED_SITE=all
	${MAKE} docker-disable-crontab
	@$(DOCKER_COMPOSE_COMMAND) exec -T web bash -c "cd $(PROJECT_PATH) && ./scripts/update-dev.sh ${SELECTED_SITE}"
	${MAKE} docker-fix-permissions
	${MAKE} docker-enable-crontab
...
docker-contrib-update: ## Update contrib. Example: make docker-contrib-update SELECTED_SITE=all
	${MAKE} docker-disable-crontab
	@$(DOCKER_COMPOSE_COMMAND) exec web bash -c "cd $(PROJECT_PATH) && ./scripts/contrib-update.sh ${SELECTED_SITE}"
	${MAKE} docker-fix-permissions
	${MAKE} docker-enable-crontab

.PHONY: docker-enable-crontab
docker-enable-crontab: ## Reload the crontab config file.
	@$(DOCKER_COMPOSE_COMMAND) exec -T drupal_cron bash -c "mkdir -p $(DRUPAL_LOG_FOLDER_PATH)"
	@$(DOCKER_COMPOSE_COMMAND) exec -T drupal_cron bash -c "touch $(DRUPAL_LOG_FOLDER_PATH)/ensure_not_empty"
	@$(DOCKER_COMPOSE_COMMAND) exec -T drupal_cron bash -c "chown -R $(WEBSERVER_USER):$(WEBSERVER_GROUP) $(DRUPAL_LOG_FOLDER_PATH)"
	@$(DOCKER_COMPOSE_COMMAND) exec -T drupal_cron bash -c "chmod -R 0755 $(DRUPAL_LOG_FOLDER_PATH)"
	@$(DOCKER_COMPOSE_COMMAND) exec -T drupal_cron bash -c "chmod -R 0644 $(DRUPAL_LOG_FOLDER_PATH)/*"
	@$(DOCKER_COMPOSE_COMMAND) exec -T drupal_cron bash -c "crontab -u root /etc/crontabs/drupal_cron"

.PHONY: docker-disable-crontab
docker-disable-crontab: ## Disable the crontab.
	@$(DOCKER_COMPOSE_COMMAND) exec -T drupal_cron bash -c "crontab -u root /etc/crontabs/empty_cron"
...
docker-fix-permissions: ## Fix directories permissions.
	@$(DOCKER_COMPOSE_COMMAND) exec -T web bash -c 'cd $(PROJECT_PATH) && \
...
generate-package: ## Generate a package. Example: make generate-package TARGET_ENVIRONMENT=prod VERSION_TYPE=branch TARGET_VERSION=10.x
	...
	@$(DOCKER_COMPOSE_COMMAND) exec web bash -c 'cd $(PROJECT_PATH) && \
	...
	@$(DOCKER_COMPOSE_COMMAND) exec web bash -c 'composer drupal:paranoia --working-dir="${PROJECT_PATH}/build/${TARGET_VERSION}"'
...
quality-psalm: ## Launch Psalm on project sources.
	@$(DOCKER_COMPOSE_COMMAND) exec web bash -c 'cd ${APP_PATH} && \
...
.PHONY: quality-fix-yaml
quality-fix-yaml: ## Launch ESLint for YAML.
	@$(DOCKER_COMPOSE_COMMAND) exec node /bin/sh -c 'cd $(PROJECT_PATH) && \
		yarn --cwd ${APP_PATH}/core install && \
		cd ${APP_PATH}/core && \
		./node_modules/.bin/eslint \
...
quality-fix-all: ## Launch all code quality fix of the project.
	...
	${MAKE} quality-fix-sass || true
	${MAKE} quality-fix-yaml
...
tests-pa11y: ## Launch Pa11y scan.
	@$(DOCKER_COMPOSE_COMMAND) run --rm pa11y_ci /bin/sh -c "pa11y-ci --config /workspace/config.json"
